"""
Send the sprite grid template to Gemini Nano Banana Pro and ask
it to fill in the cells with an SNES-style swamp monster character.
"""
...
SWAMP MONSTER character. The character design:
  • Hulking, hunched humanoid covered in dripping moss and algae
  • Mottled green-brown skin with patches of wet slime and mud
  • Glowing yellow-green eyes peering from beneath a heavy brow
  • Long, powerful arms ending in webbed claws with dark nails
  • Patches of cattails and swamp reeds growing from shoulders and back
  • A wide, frog-like mouth with jagged teeth
  • Vines and lily pads tangled around legs and torso
...
ROW 0 — Walk Down (top-down overworld, character facing the camera):
  (0,0) "Walk Down 1" — left foot forward, arms swinging, dripping slime trail
  (0,1) "Walk Down 2" — neutral standing mid-step, feet together (contact pose)
  (0,2) "Walk Down 3" — right foot forward, mirror of frame 1
  (0,3) "Walk Up 1"   — character facing away, mossy back visible, left foot forward
...
ROW 1 — Walk Left & Right (top-down overworld, side views):
  (1,0) "Walk Left 1"  — facing left, left foot forward, claws visible
  (1,1) "Walk Left 2"  — facing left, neutral contact pose
  (1,2) "Walk Left 3"  — facing left, right foot forward
  (1,3) "Walk Right 1" — facing right, right foot forward, claws visible
  (1,4) "Walk Right 2" — facing right, neutral contact pose
  (1,5) "Walk Right 3" — facing right, left foot forward

ROW 2 — Idle & Battle Idle:
  (2,0) "Idle Down"     — relaxed standing pose facing camera, arms hanging, dripping
  (2,1) "Idle Up"       — relaxed standing pose facing away, reeds swaying
  (2,2) "Idle Left"     — relaxed standing pose facing left
  (2,3) "Idle Right"    — relaxed standing pose facing right
  (2,4) "Battle Idle 1" — side-view battle stance, claws raised, slight crouch, frame 1
  (2,5) "Battle Idle 2" — battle stance with slow breathing/sway, slime dripping, frame 2

ROW 3 — Battle Idle 3, Attack sequence, Cast start:
  (3,0) "Battle Idle 3" — battle stance sway, frame 3 (loops back to frame 1)
  (3,1) "Attack 1"      — wind-up: arm pulled back, body coiled, claws spread
  (3,2) "Attack 2"      — mid-swipe: claws slashing forward, mud splatter trailing
  (3,3) "Attack 3"      — follow-through: claws fully extended, spray of swamp muck
  (3,4) "Cast 1"        — arms raised, gathering toxic swamp energy, green glow at hands
  (3,5) "Cast 2"        — casting: eyes blazing, swirling vortex of swamp gas and poison

ROW 4 — Cast 3, Damage, KO:
  (4,0) "Cast 3"   — spell release: huge toxic cloud erupting from hands, vines whipping
  (4,1) "Damage 1" — hit recoil: flinching backward, bits of moss flying off
  (4,2) "Damage 2" — stagger: leaning further back, mouth open in pain
  (4,3) "Damage 3" — recovery: stumbling forward, regaining balance, regenerating
  (4,4) "KO 1"     — collapsing: knees buckling, arms going limp
  (4,5) "KO 2"     — falling: body hitting the ground, splashing mud

ROW 5 — KO 3, Victory, Weak/Critical:
  (5,0) "KO 3"          — fully down: lying in a puddle of swamp water, eyes dim, body dissolving into mud
  (5,1) "Victory 1"     — celebration start: throwing arms up, roaring triumphantly
  (5,2) "Victory 2"     — mid-celebration: beating chest, swamp water spraying
  (5,3) "Victory 3"     — celebration end: standing tall with arms crossed, glowing eyes bright
  (5,4) "Weak Pose"     — hunched over, one knee in mud, panting, body visibly drying out and cracking
  (5,5) "Critical Pose" — desperate stance, one eye dimming, body crumbling, vines wilting
